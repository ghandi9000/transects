...
R
## Last-Updated: Wed Apr 22 13:27:01 2015 (-0400)
##           By: Noah Peart
######################################################################
require(plyr)
require(dplyr)
...
names(dat)[names(dat) %in% c("ABSRAD", "TRAD")] <- c("ABSRAD99", "TRAD99")
dat[, paste0("ABSRAD", c(87, 98, 10))] <- NA
dat[, paste0("TRAD", c(87, 98, 10))] <- NA

dat <- reshape(dat, times = yrs, direction = "long",
               varying = list(
               TRAD = grep("^TRAD", names(dat)),
               ABSRAD = grep("^ABSRAD", names(dat)),
               ...
               v.names = c("TRAD","ABSRAD","STAT", "DBH", "BV", "HT", "HTOBS", "CANHT"),
               timevar = "YEAR")
